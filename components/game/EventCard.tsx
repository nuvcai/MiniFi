...
export function EventCard({ event, onEventClick }: EventCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative overflow-hidden rounded-2xl transition-all duration-300 bg-white border-2 touch-card
          ${event.completed 
            ? "border-emerald-200 bg-gradient-to-br from-emerald-50/50 via-white to-teal-50/30" 
            : event.unlocked
              ? `border-indigo-200 hover:border-indigo-400 hover:shadow-2xl hover:${getImpactGlow()} hover:-translate-y-2 cursor-pointer`
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div className={`relative px-3.5 py-2 rounded-xl font-black text-lg transition-all duration-300 ${
                event.completed 
              ...
              {/* Difficulty with label on hover */}
              <div className="flex flex-col items-start">
                <div className="flex items-center gap-0.5">
                  {getDifficultyStars()}
                </div>
                <span className={`text-[10px] font-medium text-gray-400 transition-opacity ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
                  {getDifficultyLabel()}
                </span>
              </div>
            </div>
          ...
          <h3 className={`text-lg font-bold mb-3 leading-tight transition-colors duration-200 ${
            event.unlocked 
          ...
          {/* Market indicator + XP in one row */}
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all duration-200 ${
              ...
              <span>{event.impact === "negative" ? "Market Crash" : event.impact === "mixed" ? "Volatile" : "Bull Run"}</span>
            </div>
            
            {/* XP reward - enhanced */}
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl transition-all duration-300 ${
              isHovered && event.unlocked
                ? "bg-gradient-to-r from-amber-100 to-yellow-100 scale-110 shadow-lg shadow-amber-100/50"
                : "bg-gradient-to-r from-amber-50 to-yellow-50"
            }`}>
              <Sparkles className={`h-4 w-4 text-amber-500 transition-transform ${isHovered && event.unlocked ? "animate-pulse scale-110" : ""}`} />
              <span className="text-sm font-black text-amber-600">+{event.reward}</span>
              <span className="text-xs text-amber-500/70 font-medium">XP</span>
            </div>
          </div>

          {/* Action button - always show for consistent height */}
          <div className="mt-4">
            {event.completed ? (
              <Button
                variant="outline"
                size="sm"
                className="w-full font-semibold border-emerald-200 text-emerald-600 hover:bg-emerald-50 hover:border-emerald-300 transition-all group"
                onClick={(e) => {
                  e.stopPropagation();
                  onEventClick(event);
                }}
              ...
              <Button
                size="sm"
                className="w-full font-bold bg-gradient-to-r from-indigo-500 via-violet-500 to-purple-500 hover:from-indigo-600 hover:via-violet-600 hover:to-purple-600 text-white shadow-lg shadow-indigo-200/50 hover:shadow-xl transition-all duration-300 hover:scale-[1.02] group relative overflow-hidden"
                onClick={(e) => {
                  e.stopPropagation();
                  onEventClick(event);
                }}
              ...
              <Button
                ...
                size="sm"
                disabled
                className="w-full font-medium border-gray-200 text-gray-400 cursor-not-allowed"
              >
                <Lock className="h-4 w-4 mr-2" />
                {event.unlockDescription || "Complete previous mission"}
              </Button>
            )}
          </div>
        </div>
